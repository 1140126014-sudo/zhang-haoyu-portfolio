...
视频内容和顺序由 `src/App.jsx` 中的 `videoItems` 管理，数组第一项显示在最前面。每项包含标题、简介、封面、视频地址和时长。

将网页使用的 MP4 和 WebP 封面放在 `public/assets/videos`，再加入 `videoItems`。使用带日期或版本的新文件名，避免替换后仍命中旧缓存。原始 MOV 保留在素材目录，只提交网页播放副本。

网页副本采用 H.264 视频、AAC 音频和 MP4 faststart，保留完整时长及原始画面比例。作品列表只加载封面，打开播放器后才加载对应视频。

Cloudflare Pages 的单文件上限为 25 MiB；新增长片应压缩至该上限以内。检查脚本同时保留非视频资源 35 MiB、全部发布资源 150 MiB 的预算，并逐文件检查 25 MiB 限制。总资源预算包含按需加载的视频，不代表首屏下载量。
